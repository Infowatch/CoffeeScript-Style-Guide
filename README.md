...
#### Строки
  ...
  * Желательно использовать двойные кавычки `""`
...
#### Свойства
  * Не допускается использование свойства класса без его объявления
```coffeescript
# --------- GOOD ---------
...
class WidgetGridView
  ...
  setCurrentElement: (e) ->
    @_currentElement = e # обращаемся к неопределенному элементу

  getCurrentElement: ->
    @_currentElement

```
  * Не рекомендуется использвание объектов в качестве значений по-умолчанию для свойств, т.к. объект передаётся по ссылке, то он будет у всех экземпляров данного класса один, а значит изменение свойств затронет сразу все экземпляры.
  Чтобы избежать такого, используйте анонимные функции, отдающие нужный объект, либо присваивайте объект по-умолчанию в конструторе
```coffeescript
# --------- GOOD ---------
...
class WidgetGridView
  ...
  constructor: ->
    @currentElement = new WidgetModel
...
```
  * Для именования свойств необходимо использовать нотацию camelCase
  * Имя свойства должно нормально переводиться на русский язык и использовать правила построения предложений в английском языке
  * Имена приватных свойств, функций и методов должны начинаться с символа подчеркивания _
